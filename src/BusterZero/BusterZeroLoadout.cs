...
[ProtoContract]
public class BZeroLoadout {
	[ProtoMember(1)]
	public int gigaAttack;
	[ProtoMember(2)]
	public int hyperMode;

	public static BZeroLoadout createRandom() {
		return new BZeroLoadout() {
			gigaAttack = Helpers.randomRange(0, 2),
			hyperMode = Helpers.randomRange(0, 2)
		};
	}

	internal BZeroLoadout clone() {
		return new BZeroLoadout() {
...
public class SelectBusterZeroWeaponMenu : IMainMenu {
	// Menu controls.
	...
	public SelectBusterZeroWeaponMenu(IMainMenu prevMenu, bool inGame) {
		this.prevMenu = prevMenu;
		this.inGame = inGame;
		hyperMode = Options.main.bzeroLoadout.hyperMode;
		gigaAttack = Options.main.bzeroLoadout.gigaAttack;
	}
	...
	public void update() {
		...
		if (okPressed || backPressed && !inGame) {
			Options.main.bzeroLoadout.gigaAttack = gigaAttack;
			Options.main.bzeroLoadout.hyperMode = hyperMode;
	...
	public void render() {
		...
		if (cursorRow == 1) {
			...
			weaponDescription = hyperMode switch {
				0 => "Increases speed by 50%" +
					"\nIncreases charge speed by 15%" +
					"\nIncreases saber damage by 1.",
				1 => "Grants the Shin variants of attacks\ngiving them pierce and homming.",
				2 => "Applies 100% damage as viral to enemies on hit" +
					"\nand 50% when they hit you." +
					"\n",
				_ => "ERROR"
			};
			weaponSubDescription = hyperMode switch {
				0 => "",
				1 => "",
				2 => "Viral debuff: Slows time on enemies up to 75%",
				_ => "ERROR"
			};
		}
