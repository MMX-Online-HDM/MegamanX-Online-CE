...
public partial class MegamanXA : Character {
	...
	public override Projectile? getMeleeProjById(int id, Point projPos, bool addToLevel = true) {
		...
		return id switch {
			(int)MeleeIds.SpeedBurnerCharged => new GenericMeleeProj(
				new SpeedBurner(), projPos, ProjIds.SpeedBurnerCharged, player
			),
			(int)MeleeIds.Shoryuken => new GenericMeleeProj(
				new ShoryukenWeapon(), projPos, ProjIds.Shoryuken, player
			),
			(int)MeleeIds.X3Saber => new GenericMeleeProj(
				new ZXSaber(), projPos, ProjIds.XSaber, player
			),
			(int)MeleeIds.X6Saber => new GenericMeleeProj(
				new ZXSaber(), projPos, ProjIds.X6Saber, player,
				damage: grounded ? 3 : 2, flinch: 0
			),
			(int)MeleeIds.NovaStrike => new GenericMeleeProj(
				new HyperNovaStrike(), projPos, ProjIds.NovaStrike, player
			),
